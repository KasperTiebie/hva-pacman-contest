...
class offensiveAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    def alphabeta(state, depth, agent, alpha, beta, desiredDepth, desiredAgent):
      # If the game is over at this gamestate
      if state.isOver():
        return self.evaluationFunction(state)

      # If the algorithm is at the desired depth and agent
      if agent is desiredAgent and depth is desiredDepth:
        return self.evaluationFunction(state)

      # If this is the last agent, go one level deeper
      if agent is state.getNumAgents():
      ...
      if agent in self.getTeam(state):  # If agent is max
        ...
        value = float('inf')
        if state.getAgentPosition(agent) is not None: # If the agent is observable
          for action in state.getLegalActions(agent):
            successor = state.generateSuccessor(agent, action)
            ...
            if value < alpha:  # If the value is lower than MAX's best option on path to root, return the value
              return value
            beta = min([beta, value])
        else: # Not observable? Skip the agent
    ...
    for action in gameState.getLegalActions(self.index):
      ...
      # If the value is higher than the highest value registered
      if value > alpha:
        alpha = value
        bestActions = []
  ...
  def evaluationFunction(self, currentGameState):
    """
      The evaluation function of the offensive ghost
      distanceToHome: The distance to the home position
      minimumFoodDistance: The distance to the nearest food pellet
      foodLeft: The amount of food to eat left
      ghostDistance: The distance to the nearest ghost
      foodCarrying: The amount of food the agent is currently carrying
      foodReturned: The total amount of food the team has returned
    """
...
class defensiveAgent(CaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    # Draw the patrol area (green) and the middle of the board (red)
        ...
        if state.getAgentPosition(agent) is not None:
          for action in state.getLegalActions(agent):
            ...
            if value < alpha:  # If the value is lower than MAX's best option on path to root, return the value
              return value
            beta = min([beta, value])
        else: # Not observable? Skip agent
          value = alphabeta(state, depth, agent + 1, alpha, beta, desiredDepth, desiredAgent)
  ...
  def evaluationFunction(self, currentGameState):
    """
      The evaluation function for the defensive agent
      foodDefendingLeft: The amount of food left there is to defend
      distanceToEnemy: The distance to the nearest enemy
      isPacman: If the agent is a pacman at this state
      distanceToPatrolArea: The minimum distance to the patrol area
    """
    ...
    if agentPosition[0] < self.patrolAreaX[0] or agentPosition[0] > self.patrolAreaX[1] or \
            agentPosition[1] < self.patrolAreaY[0] or agentPosition[1] > self.patrolAreaY[1]:
      # Loop through the left and right side of the patrol area and calculate the distance to each square
      ...
      # Loop through the top and bottom side of the patrol area and calculate the distance to each square
